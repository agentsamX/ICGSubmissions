...
void main() {
	...
	vec3 textureColor = (u_TextureOn==1)?texture(textureSampler, UV).xyz:vec3(0.7f);
	...
	vec3 result = round((ambient+diffuse+specular)*u_toonBands)/u_toonBands;

	/*if(dot(inNormal,camDir)<0.01)
	{
